    ...
    def __init__(self, Profile, Password):
        ...
        try:
            ...
            with open(f'{os.path.dirname(__file__)}/password-data/{self.profile}/Profilepass.txt', 'r') as file:
                ...
                assert Verify_password == self.Password, "Verification fail wrong password Terminate the program"
...
class Account(App):
    ...
    def encoding(self):
        ...
        EncodedPassword2 = list(map(lambda x : str(x).encode('utf-16'), EncodedPassword1))
        EncodedPassword3 = [base64.b64encode(_).decode('utf-8') for _ in EncodedPassword2]
        ...
        if Account_mode != 'a' and Account_mode != 's':
            ...
            if Account_mode == 'a':
                ...
                try:
                    ...
                    Account_list = [os.path.splitext(file)[0] for file in Account_file_list]
...
def main():
    ...
    New_account = Account(Profile=Profile, ProfilePass=Profile_password, Appname=App, Username=Account_name, Password=Account_password)
